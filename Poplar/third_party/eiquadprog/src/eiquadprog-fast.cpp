...
// #define OPTIMIZE_ADD_CONSTRAINT
      ...
      */
        ...
        void
        EiquadprogFast::delete_constraint(MatrixXd &R, MatrixXd &J, VectorXi &A, VectorXd &u, size_t nEqCon, size_t &iq,
                                          size_t l) {
            size_t nVars = R.rows();
#ifdef TRACE_SOLVER
        ...
        EiquadprogFast::solve_quadprog(const MatrixXd &Hess, const VectorXd &g0, const MatrixXd &CE,
         ...
         */
          ...
          x = -x;
            ...
            if (std::abs(psi) <=
                static_cast<double>(nIneqCon) * std::numeric_limits<double>::epsilon() * c1 * c2 * 100.0) {
            ...
            if (t == t2) {
                ...
                if (!add_constraint(R, m_J, d, iq, R_norm)) {
                    ...
                    for (i = 0; i < iq; i++) {
                        A(i) = A_old(i);
                        if (A(i) < 0) {
                            return EIQUADPROG_FAST_INFEASIBLE;
                        }
                        iai(A(i)) = -1;
                        u(i) = u_old(i);
                    }
