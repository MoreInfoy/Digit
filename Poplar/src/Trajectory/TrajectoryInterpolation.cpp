...
SamplePoint TrajectoryInterpolation::operator()(double t) {
    ...
    for (int i = 0; i < _dims; i++) {
        ...
        samplePoint.first_derivative()(i) = n_lines[i_piece][i].deriv(Spline::first_deriv, t);
        samplePoint.second_derivative()(i) = n_lines[i_piece][i].deriv(Spline::second_deriv, t);
    }
    return samplePoint;
}

SamplePoint TrajectoryInterpolation::operator()(double t, size_t dim_num) {
    int i_piece;
    for (i_piece = 0; i_piece < end_t.size(); i_piece++) {
        if (t < end_t[i_piece]) break;
    }
    ...
    samplePoint.t() = t;
    samplePoint.data()(0) = n_lines[i_piece][dim_num](t);
    samplePoint.first_derivative()(0) = n_lines[i_piece][dim_num].deriv(Spline::first_deriv, t);
    samplePoint.second_derivative()(0) = n_lines[i_piece][dim_num].deriv(Spline::second_deriv, t);
    return samplePoint;
}
...
bool TrajectoryInterpolation::setSamplePoints(vector<SamplePoint> &samplePoints) {
    ...
    n_lines.clear();
    end_t.clear();
    vector<vector<Scalar>> data(_dims);
    vector<Scalar> t;
    vector<vector<Scalar>> boundary_der(_dims);
    ...
    for (auto &point: samplePoints) {
        ...
        if (point.pointType() == SamplePoint::POINTTYPE::ENDPOINT) {
            for (int i = 0; i < _dims; i++) {
                boundary_der[i].push_back(point.first_derivative()(i));
            }
            if (first_point) {
                first_point = false;
            } else {
