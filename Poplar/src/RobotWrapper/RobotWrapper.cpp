...
ConstVecRef RobotWrapper::homeConfigurations() {
    if (has_srdf) {
...
ConstVecRef RobotWrapper::gear_ratio() {
    if (has_srdf) {
...
ConstVecRef RobotWrapper::rotorInertia() {
    if (has_srdf) {
...
ConstVecRef RobotWrapper::actuatorsEffortLimit() {
    return ConstVecRef(_model.effortLimit.tail(na()));
}

ConstVecRef RobotWrapper::actuatorsDamping() {
    return ConstVecRef(_model.damping.tail(na()));
}

ConstVecRef RobotWrapper::actuatorsFriction() {
    return ConstVecRef(_model.friction.tail(na()));
}
...
ConstVecRef RobotWrapper::actuatorsDampingForce() {
    return _actuatorsDampingForce;
}


ConstVecRef RobotWrapper::jointsSpringForce() {
...
ConstMatRef RobotWrapper::M() {
    return ConstMatRef(_data.M);
}

ConstMatRef RobotWrapper::Minv() {
    return ConstMatRef(_data.Minv);
}

ConstVecRef RobotWrapper::nonLinearEffects() {
    return ConstVecRef(_data.nle);
}
...
ConstMatRef RobotWrapper::contactJacobia() {
    assert(_mask.size() == _contactPoint_virtual_link.size());
    return ConstMatRef(_Jc);
}
...
ConstVecRef RobotWrapper::activeContactPointBiasAcc() {
    return ConstVecRef(_contactPointsBiasAcc);
}

ConstVecRef RobotWrapper::qpos() {
    return ConstVecRef(_qpos);
}

ConstVecRef RobotWrapper::qvel() {
    return ConstVecRef(_qvel);
}
...
void RobotWrapper::setConstraintForceSubspace(ConstMatRef T, ConstMatRef T_dot) {
    _T = T;
    _T_dot = T_dot;
}
...
ConstMatRef RobotWrapper::constraintForceJacobia() {
    return ConstMatRef(_K);
}

ConstVecRef RobotWrapper::connectPointBiasAcc() {
    return ConstVecRef(_connectPointsBiasAcc);
}
...
ConstMatRef RobotWrapper::connectPointRelativeJacobia() {
    return ConstMatRef(_Jps);
}
