...
namespace tk {
    ...
    std::vector<RealNum> Spline::get_x() const { return m_x; }

    std::vector<RealNum> Spline::get_y() const { return m_y; }

    RealNum Spline::get_x_min() const {
        assert(!m_x.empty());
        return m_x.front();
    }

    RealNum Spline::get_x_max() const {
        assert(!m_x.empty());
        return m_x.back();
    }

    void Spline::set_boundary(Spline::bd_type left, RealNum left_value,
                              Spline::bd_type right, RealNum right_value) {
    ...
    void Spline::set_points(const std::vector<RealNum> &x,
        ...
        if (type == linear) {
            ...
            for (int i = 1; i < n - 1; i++) {
                const RealNum h = m_x[i + 1] - m_x[i];
                const RealNum hl = m_x[i] - m_x[i - 1];
            ...
            if (m_left == first_deriv) {
                ...
                const RealNum h = m_x[1] - m_x[0];
                m_b[0] = 0.5 * (-m_b[1] - 0.5 * m_left_value * h + 3.0 * (m_y[1] - m_y[0]) / h);
            } else if (m_left == not_a_knot) {
                // f''' continuous at x[1]
                const RealNum h0 = m_x[1] - m_x[0];
                const RealNum h1 = m_x[2] - m_x[1];
            ...
            if (m_right == first_deriv) {
                ...
                const RealNum h = m_x[n - 1] - m_x[n - 2];
                ...
                const RealNum h0 = m_x[n - 2] - m_x[n - 3];
                const RealNum h1 = m_x[n - 1] - m_x[n - 2];
    ...
    bool Spline::make_monotonic() {
        ...
        for (int i = 0; i < n - 1; i++) {
            RealNum h = m_x[i + 1] - m_x[i];
            RealNum avg = (m_y[i + 1] - m_y[i]) / h;
            if (avg == 0.0 && (m_b[i] != 0.0 || m_b[i + 1] != 0.0)) {
                ...
                RealNum r = sqrt(m_b[i] * m_b[i] + m_b[i + 1] * m_b[i + 1]) / std::fabs(avg);
                if (r > 3.0) {
...
// return the closest idx so that m_x[idx] <= x (return 0 if x<m_x[0])
    size_t Spline::find_closest(RealNum x) const {
        std::vector<RealNum>::const_iterator it;
        it = std::upper_bound(m_x.begin(), m_x.end(), x);       // *it > x
        size_t idx = std::max(int(it - m_x.begin()) - 1, 0);   // m_x[idx] <= x
        return idx;
    }

    RealNum Spline::operator()(RealNum x) const {
    ...
    RealNum Spline::deriv(int order, RealNum x) const {
        assert(order > 0);
        size_t n = m_x.size();
        size_t idx = find_closest(x);
    ...
    std::vector<RealNum> Spline::solve(RealNum y, bool ignore_extrapolation) const {
        std::vector<RealNum> x;          // roots for the entire Spline
        std::vector<RealNum> root;       // roots for each piecewise cubic
        ...
        for (size_t i = 0; i < n - 1; i++) {
            ...
            for (size_t j = 0; j < root.size(); j++) {
                RealNum h = (i > 0) ? (m_x[i] - m_x[i - 1]) : 0.0;
                RealNum eps = internal::get_eps() * 512.0 * std::min(h, 1.0);
                if ((-eps <= root[j]) && (root[j] < m_x[i + 1] - m_x[i])) {
                    RealNum new_root = m_x[i] + root[j];
...
// by A(i,j), index going from i=0,...,dim()-1
        RealNum &band_matrix::operator()(int i, int j) {
        ...
        RealNum band_matrix::operator()(int i, int j) const {
            int k = j - i;       // what band is the entry
            assert((i >= 0) && (i < dim()) && (j >= 0) && (j < dim()));
            assert((-num_lower() <= k) && (k <= num_upper()));
            // k=0 -> diagonal, k<0 lower left part, k>0 upper right part
...
// second diag (used in LU decomposition), saved in m_lower
        RealNum band_matrix::saved_diag(int i) const {
            assert((i >= 0) && (i < dim()));
            return m_lower[0][i];
        }

        RealNum &band_matrix::saved_diag(int i) {
            assert((i >= 0) && (i < dim()));
            return m_lower[0][i];
        }
...
// solves Ly=b
        std::vector<RealNum> band_matrix::l_solve(const std::vector<RealNum> &b) const {
            assert(this->dim() == (int) b.size());
            std::vector<RealNum> x(this->dim());
            int j_start;
            RealNum sum;
            for (int i = 0; i < this->dim(); i++) {
                sum = 0;
...
// solves Rx=y
        std::vector<RealNum> band_matrix::r_solve(const std::vector<RealNum> &b) const {
            assert(this->dim() == (int) b.size());
            std::vector<RealNum> x(this->dim());
            int j_stop;
            RealNum sum;
        ...
        std::vector<RealNum> band_matrix::lu_solve(const std::vector<RealNum> &b,
                                                   bool is_lu_decomposed) {
            assert(this->dim() == (int) b.size());
            std::vector<RealNum> x, y;
        ...
        RealNum get_eps() {
            //return std::numeric_limits<RealNum>::epsilon();    // __DBL_EPSILON__
            return 2.2204460492503131e-16;                      // 2^-52
        }

        std::vector<RealNum> solve_linear(RealNum a, RealNum b) {
            std::vector<RealNum> x;      // roots
            if (b == 0.0) {
                if (a == 0.0) {
                    // 0*x = 0
                    x.resize(1);
...
// solutions for a + b*x + c*x^2 = 0
        std::vector<RealNum> solve_quadratic(RealNum a, RealNum b, RealNum c,
                                             int newton_iter = 0) {
            if (c == 0.0) {
            ...
            RealNum q = a / c;
            RealNum discr = p * p - q;
            const RealNum eps = 0.5 * internal::get_eps();
            RealNum discr_err = (6.0 * (p * p) + 3.0 * fabs(q) + fabs(discr)) * eps;
            ...
            for (size_t i = 0; i < x.size(); i++) {
                for (int k = 0; k < newton_iter; k++) {
                    RealNum f = (c * x[i] + b) * x[i] + a;
                    RealNum f1 = 2.0 * c * x[i] + b;
...
//   octave: roots.m - via eigenvalues of the Frobenius companion matrix
        std::vector<RealNum> solve_cubic(RealNum a, RealNum b, RealNum c, RealNum d,
            ...
            std::vector<RealNum> z;              // roots of the depressed cubic
            RealNum p = -(1.0 / 3.0) * b + (1.0 / 9.0) * (c * c);
            RealNum r = 2.0 * (c * c) - 9.0 * b;
            RealNum q = -0.5 * a - (1.0 / 54.0) * (c * r);
            RealNum discr = p * p * p - q * q;             // discriminant
            ...
            // would be more elegant to use boost::numeric::interval<RealNum>
            const RealNum eps = internal::get_eps();
            RealNum p_err = eps * ((3.0 / 3.0) * fabs(b) + (4.0 / 9.0) * (c * c) + fabs(p));
            RealNum r_err = eps * (6.0 * (c * c) + 18.0 * fabs(b) + fabs(r));
            RealNum q_err = 0.5 * fabs(a) * eps + (1.0 / 54.0) * fabs(c) * (r_err + fabs(r) * 3.0 * eps)
                            + fabs(q) * eps;
            RealNum discr_err = (p * p) * (3.0 * p_err + fabs(p) * 2.0 * eps)
            ...
            if (fabs(discr) <= discr_err) {
                ...
                if (fabs(p) <= p_err) {
                    ...
                    z[1] = -0.5 * z[0];       // RealNum root
                }
            } else if (discr > 0) {
                // three real roots: via trigonometric solution
                z.resize(3);
                RealNum ac = (1.0 / 3.0) * acos(q / (p * sqrt(p)));
                RealNum sq = 2.0 * sqrt(p);
                z[0] = sq * cos(ac);
                ...
                z.resize(1);
                RealNum sgnq = (q >= 0 ? 1 : -1);
                RealNum basis = fabs(q) + sqrt(-discr);
                RealNum C = sgnq * pow(basis, 1.0 / 3.0); // c++11 has std::cbrt()
                z[0] = C + p / C;
            }
            for (size_t i = 0; i < z.size(); i++) {
                ...
                // improve solution via newton steps
                for (int k = 0; k < newton_iter; k++) {
                    RealNum f = ((z[i] + c) * z[i] + b) * z[i] + a;
                    RealNum f1 = (3.0 * z[i] + 2.0 * c) * z[i] + b;
                    // only adjust if slope is large enough
                    if (fabs(f1) > 1e-8) {
                        z[i] -= f / f1;
                    }
