...
#ifdef FIXED_BASE
/*double q_init[] = {0.325685, 0.003982, 0.060953, 0.998499, -0.002613, 0.000547, 0.054705, 0.109731, 0, -0.109664, 0, 0.027702,
                   0.999890, -0.004864, 0.001440, -0.013921, -0.061125, 0.999420, 0.011867, -0.002226, 0.031841,
                   -0.046928, -0.057329, -0.308679, 0.937404, -0.037069, 0.020408, -0.325474, -0.000611, -0.062242,
                   0.998510, 0.002614, 0.000544, -0.054497, -0.109314, 0, 0.109222, 0, -0.026694, 0.999897, 0.004863,
                   0.001417, 0.013403, 0.060098, 0.999437, -0.011867, -0.002203, -0.031310, 0.045831, 0.057292,
                   0.309157, -1.012092, 0.001563, 0.019892};*/

double q_init[] = {0.325685, 0.003982, 0.060953, 0.998499, -0.002613, 0.000547, 0.054705, 0.109731,  -0.109664,  0.027702,
                   0.999890, -0.004864, 0.001440, -0.013921, -0.061125, 0.999420, 0.011867, -0.002226, 0.031841,
                   -0.046928, -0.057329, -0.308679, 0.937404, -0.037069, 0.020408, -0.325474, -0.000611, -0.062242,
                   0.998510, 0.002614, 0.000544, -0.054497, -0.109314,  0.109222,  -0.026694, 0.999897, 0.004863,
                   0.001417, 0.013403, 0.060098, 0.999437, -0.011867, -0.002203, -0.031310, 0.045831, 0.057292,
                   0.309157, -1.012092, 0.001563, 0.019892};
...
#else
/*double q_init[] = {-0.000064, 0.000070, 0.950181, 0.999999, 0.000030, -0.000027, 0.001287,
...
void infotext(char *title, char *content, double interval) {
    ...
    sprintf(content, "%-20.3f\n%d  (%d con)\n%.3f\n%.1f  (%d it)\n%.0f\n%.3f\n%.3f\n%.3f",
            d->time,
            d->nefc, d->ncon,
            settings.run ? d->timer[mjTIMER_STEP].duration / mjMAX(1, d->timer[mjTIMER_STEP].number) :
...
void loadmodel(void) {
    ...
    if (shared_memory.is_created()) {
        ...
        //TODO: write some meesage in shared memory
    ...
    gear.resize(m->nu);
    tau_user.resize(m->nu);
    tau_user.setZero();
    for (int i = 0; i < m->nu; i++) {
        gear(i) = m->actuator_gear[6 * i];
    }
    ...
    // enable contact force and contact point visualization
    vopt.flags[mjtVisFlag::mjVIS_CONTACTFORCE] = 1;
    vopt.flags[mjtVisFlag::mjVIS_CONTACTPOINT] = 1;
...
//    mju_printMat(d->qpos, 1, m->nq);
    ...
    for (int i = 0; i < d->ncon; i++) {
        printf("---------------------c%d--------------------\n", i);
        Poplar::Vec3 pos, pos_ankle, pos_r;
        Poplar::Mat3 rot, c_frame;
        pos << d->contact[i].pos[0], d->contact[i].pos[1], d->contact[i].pos[2];
        mju_copy(rot.data(), d->xmat + 9 * mj_name2id(m, mjOBJ_BODY, "left_toe_roll"), 9);
        mju_copy(c_frame.data(), d->contact[i].frame, 9);
        mju_copy(pos_ankle.data(), d->xpos + 3 * mj_name2id(m, mjOBJ_BODY, "left_toe_roll"), 3);
        pos_r = rot.transpose() * (pos - pos_ankle);
        mju_printMat(rot.data(), 3, 3);
        mju_printMat(pos.data(), 1, 3);
        mju_printMat(pos_ankle.data(), 1, 3);
        mju_printMat(pos_r.data(), 1, 3);
        Poplar::Vec quat(4);
        mju_mat2Quat(quat.data(), rot.data());
        mju_printMat(quat.data(), 1, 4);
        std::cout << rot.eulerAngles(2,1,0).transpose() << std::endl;
        printf("-----------------------------------------\n");
    }
     */
}
